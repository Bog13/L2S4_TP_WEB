...
/**
 * Affiche les rendez vous de l'utilisateur dont l'identfiant est passé par l'url
 */
function bog_display_rdv_url()
{
    //error management
    if( !isset($_GET['IDUser']) )
        {
            echo 'E: undefined IDUser';
            return;
        }

    if( $_GET['IDUser'] < 0 || $_GET['IDUser'] >= 999999 )
        {
            echo 'E: IDUser must be between 0 and 999999';
            return;
        }
    
    if( !is_numeric($_GET['IDUser']) )
        {
            echo 'E: IDUser must be a numerical value';
            return;
        }


    bog_display_rdv_by_id( $_GET['IDUser'] );
